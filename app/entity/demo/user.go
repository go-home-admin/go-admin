...
func (orm *OrmUsers) Pluck(column string, dest interface{}) *gorm.DB {
	return orm.db.Model(&Users{}).Pluck(column, dest)
}
...
func (orm *OrmUsers) Count() int64 {
	var count int64
	orm.db.Model(&Users{}).Count(&count)
	return count
}
...
func (orm *OrmUsers) Find(conds ...interface{}) (UsersList, int64) {
	list := make([]*Users, 0)
	tx := orm.db.Model(&Users{}).Find(&list, conds...)
	if tx.Error != nil {
